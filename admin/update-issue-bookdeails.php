...
if (strlen($_SESSION['alogin']) == 0) {
    ...
    if (isset($_POST['issue'])) {
        ...
        if ($rowCount > 0) {
            ...
            $consultaSql = "SELECT * FROM tblissuedbookdetails WHERE id = :id";
            $consultaQuery = $dbh->prepare($consultaSql);
            $consultaQuery->bindParam(':id', $newId, PDO::PARAM_INT);
            $consultaQuery->execute();
            $consultaResult = $consultaQuery->fetch(PDO::FETCH_ASSOC);

            $studentname = $consultaResult['studentname'];
            $nombre = $consultaResult['nombre'];
            $horaEmision = $consultaResult['horaEmision'];
            $horaRegreso = $consultaResult['horaRegreso'];
            ...
            $pdf->Cell(0, 10, $horaEmision, 0, 1);
            $pdf->Cell(40, 10, 'Fecha y hora de regreso:', 0, 0);
            $pdf->Cell(0, 10, $horaRegreso, 0, 1);
...
<button type="button" id="update" class="btn btn-success" style="display:none;">Actualizar libro</button>
                            </form>
                        </div>
                    </div>
                </div>
...
<script>
        // Llamada AJAX para obtener la información del libro emitido
        $(document).ready(function() {
            $('#submit').click(function() {
                var studentname = $('#studentname').val();
                var nombre = $('#nombre').val();
                var horaEmision = $('#horaEmision').val();
                var horaRegreso = $('#horaRegreso').val();
                
                $.ajax({
                    url: 'get-issued-book.php',
                    type: 'post',
                    data: {
                    ...
                    success: function(response) {
                        if (response.success) {
                            $('#update').show();
                            $('#update').attr('data-id', response.id);
                        } else {
                            $('#update').hide();
                        }
                    }
